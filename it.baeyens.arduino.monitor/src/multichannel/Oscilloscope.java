...
public class Oscilloscope extends Canvas {
	...
	public Oscilloscope(int channels, Composite parent, int style, Color backgroundColor, Color foregroundColor,
			Color gridColor) {
	...
	public Oscilloscope(int channels, OscilloscopeDispatcher dispatcher, Composite parent, int style,
			Color backgroundColor, Color foregroundColor, Color gridColor) {
	...
	protected int ConvertValueToScreenPosition(int Value, int ScreenHeight) {
		...
		float ret = ((float) (Value - this.myLowRangeValue) / (float) (this.myhighValue - this.myLowRangeValue))
				* ScreenHeight;
	...
	/**
	 * This method sets the view of the monitor This range is used for all
	 * channels which are not set on percentage To go back to default set
	 * lowValue and highValue back to 0 This method invalidates the widget so it
	 * gets redrawn.
	 * 
	 * @param lowValue
	...
	private Image CreateBackgroundImage(int numHorizontalLines, int numVerticalLines) {
		...
		int RangeStep = (int) ((float) (rect.height - 20) / (float) (numHorizontalLines - 1)
				* (this.myhighValue - this.myLowRangeValue) / rect.height);
	...
	private void setTailSizeInternal(int channel) {
		...
		if (this.chan[channel].originalTailSize == TAILSIZE_DEFAULT) {
			...
			if (isSteady(channel)) {
				...
				|| this.chan[channel].originalTailSize > this.chan[channel].width) {
			// tail = new int[width - 2 + 1];
			this.chan[channel].tailSize = this.chan[channel].width - 2;
